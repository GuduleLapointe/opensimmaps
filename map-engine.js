/**
 * OpenSimMaps main javascript.
 *
 * Parameters are saved in config.js, which must be loaded before this script.
 */
  ...
  for(var i=0;i<__items.length;i++){
    if(__items[i].nodeType == 1){
      ...
      if(xmllocX==x&&xmllocY==y){
        ...
        response+="<tr>";
        response+="<td><a class=\"add\" href=\"hop://"+hgdomains[map.getMapTypeId()]+portstring+"/"+xmlregionname+"/"+xjump+"/"+yjump+"/\">Hop</a>&nbsp;&nbsp;</td>";
        response+="<td><a class=\"add\" href=\"secondlife://"+hgdomains[map.getMapTypeId()]+portstring+":"+xmlregionname+"/"+xjump+"/"+yjump+"/\">Hypergrid</a>&nbsp;&nbsp;</td>";
